...
for Q in $QPS_TARGETS; do
  echo "=== 8B N-calls Gen openloop qps=$Q ==="
  ...
  python benchmark_sglang_jev_openloop.py --dataset-dir $D --qps $Q --duration $DUR \
    --server http://localhost:30000 --output results_openloop_gen_ncalls_8b_qps$Q
done
for Q in $QPS_TARGETS; do
  echo "=== 8B Batched-Completions openloop qps=$Q ==="
  curl -s -X POST http://localhost:30000/flush_cache > /dev/null
  python benchmark_sglang_batched_completions_openloop.py --dataset-dir $D --model-path $MODEL --qps $Q --duration $DUR \
    --server http://localhost:30000 --output results_openloop_batched_completions_8b_qps$Q
done
for Q in $QPS_TARGETS; do
  echo "=== 8B SIS openloop qps=$Q ==="
  curl -s -X POST http://localhost:30000/flush_cache > /dev/null
  python benchmark_sglang_score_api_openloop.py --dataset-dir $D --model-path $MODEL --qps $Q --duration $DUR \
    --flush-cache-interval 5 --server http://localhost:30000 --output results_openloop_sis_8b_qps$Q
done
echo GENSIDE_8B_OPENLOOP_DONE
