...
# --chunked-prefill-size -1, NO --enable-mis). Uses --workers-per-10-qps 2
...
for Q in $QPS_TARGETS; do
  ...
  python benchmark_sglang_setwise_openloop.py --dataset-dir $D --model-path $MODEL --qps $Q --duration $DUR \
    --workers-per-10-qps 2 --server http://localhost:30000 --output results_openloop_setwise_4b_qps$Q
done
